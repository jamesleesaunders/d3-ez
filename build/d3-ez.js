 ...
 */
d3.ez.chart.barChartStacked = function module() {
    // SVG and Chart containers (Populated by 'my' function)
    var svg;
    var chart;
    // Default Options (Configurable via setters)
...
d3.ez.chart.candlestickChart = function module() {
    ...
    function init(data) {
        ...
        // X & Y Scales
        xScale = d3.scaleTime().domain([ new Date(minDate - 864e5), new Date(maxDate + 864e5) ]).range([ 0, chartW ]);
    ...
    function my(selection) {
        selection.each(function(data) {
            ...
            var candleSticks = d3.ez.component.candleSticks().width(chartW).height(chartH).xScale(xScale).yScale(yScale).colorScale(colorScale).dispatch(dispatch);
